...
def carregar_dados_consenso_b3():
    dados = {
        ...
        # --- MID CAPS, SMALL CAPS & RADAR DE POTENCIAL ---
...
def calcular_opcao_b3_dinamica(ticker, preco_acao, tipo_opcao="Call"):
    ...
    Para CALL: Strike ~10% acima
    Para PUT: Strike ~10% abaixo
    """
    hoje = datetime.now()
    ...
    # Letras para CALL e PUT (B3 usa convenção diferente)
    if tipo_opcao == "Call":
...
if "carteira_operacoes" not in st.session_state:
    st.session_state.carteira_operacoes = {}  # {ticker: [{"id": uuid, "tipo": "Ação"/"Call"/"Put", ...}]}

banco_b3 = carregar_dados_consenso_b3()
lista_tickers = list(banco_b3.keys())

with st.spinner("Atualizando base quantitativa completa da B3 (Todos os papéis)..."):
...
# --- SEÇÃO: CUSTOMIZAR CÓDIGO DE OPÇÃO ---
st.markdown("---")
st.write("#### 🔧 Customização de Código de Opção (se diferente do recomendado)")
...
with col_btn1:
    if st.button("💾 Gravar Operação", use_container_width=True, key="btn_gravar"):
        if qtd_op > 0:
            ...
            # Adiciona à carteira (múltiplas operações por ticker)
...
for ticker, info in banco_b3.items():
    ...
    # Se modo "Apenas Minha Carteira", filtra
    if modo_exibicao == "Apenas Minha Carteira" and not tem_operacoes:
        continue
    
    # Se não tem operações, cria linha com dados do mercado apenas
    if not tem_operacoes:
        ...
        # Se tem operações, cria uma linha para cada operação
