...
@st.cache_data(ttl=120)  # Reduzido para 2 minutos para atualizar mais rápido
def obter_preco_atual(ticker):
    """Obtém preço via yfinance forçando o histórico recente"""
    try:
...
def calcular_preco_com_status(ticker, preco_compra):
    """Retorna o preço e um indicador de Status (API ou Fallback)"""
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS usuarios (
            id TEXT PRIMARY KEY, email TEXT UNIQUE NOT NULL, password_hash TEXT NOT NULL, name TEXT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)''')
    c.execute('''CREATE TABLE IF NOT EXISTS operacoes (
            id TEXT PRIMARY KEY, user_id TEXT NOT NULL, ticker TEXT NOT NULL, tipo_ativo TEXT NOT NULL, qtd INTEGER NOT NULL, preco_compra REAL NOT NULL, codigo_opcao TEXT, status TEXT DEFAULT 'Aberta', data_operacao TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(user_id) REFERENCES usuarios(id))''')
    conn.commit()
    conn.close()

init_db()

# ============ FUNÇÕES AUTH E OPERAÇÕES (MANTIDAS IGUAIS) ============
...
def registrar_usuario(email, password, name):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute('INSERT INTO usuarios (id, email, password_hash, name) VALUES (?, ?, ?, ?)', (str(uuid.uuid4()), email, hash_password(password), name))
        conn.commit()
        conn.close()
        return True
...
def login_usuario(email, password):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute('SELECT id, email, name FROM usuarios WHERE email = ? AND password_hash = ?', (email, hash_password(password)))
        user = c.fetchone()
        conn.close()
        if user: return {"id": user[0], "email": user[1], "name": user[2]}
        return None
    except:
        return None
...
def salvar_operacao(user_id, ticker, tipo_ativo, qtd, preco_compra, codigo_opcao):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute('INSERT INTO operacoes (id, user_id, ticker, tipo_ativo, qtd, preco_compra, codigo_opcao) VALUES (?, ?, ?, ?, ?, ?, ?)', (str(uuid.uuid4()), user_id, ticker, tipo_ativo, qtd, preco_compra, codigo_opcao))
        conn.commit()
        conn.close()
        return True
    except:
...
def carregar_operacoes(user_id):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute('SELECT id, ticker, tipo_ativo, qtd, preco_compra, codigo_opcao, status, data_operacao FROM operacoes WHERE user_id = ? ORDER BY data_operacao DESC', (user_id,))
        rows = c.fetchall()
        conn.close()
        return [{"id": r[0], "ticker": r[1], "tipo_ativo": r[2], "qtd": r[3], "preco_compra": r[4], "codigo_opcao": r[5], "status": r[6], "data_operacao": r[7]} for r in rows]
    except:
        return []
...
def atualizar_operacao(op_id, status, preco_compra, qtd):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute('UPDATE operacoes SET status=?, preco_compra=?, qtd=? WHERE id = ?', (status, preco_compra, qtd, op_id))
        conn.commit()
        conn.close()
        return True
    except:
...
if st.session_state.user is None:
    ...
    with col2:
        ...
        with tab2:
            ...
            if st.button("Cadastrar", use_container_width=True):
                if not nome or not email or not password: st.error("Preencha todos os campos")
                elif password != password_confirm: st.error("Senhas não coincidem")
                elif registrar_usuario(email, password, nome): st.success("Cadastro realizado!")
                else: st.error("Email já existe")
else:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Operações (Boleta)", "Recomendações B3", "Minha Carteira", "Relatórios"])
    ...
    with tab1:
        ...
        with col1:
            ticker_selecionado = st.selectbox("Ação Base:", sorted([t.replace(".SA", "") for t in banco_b3.keys()]), key="ticker_op")
        with col2:
            tipo = st.selectbox("Tipo:", ["Ação Pura", "Call", "Put"], key="tipo_op")
        with col3:
            qtd = st.number_input("Qtd:", min_value=1, value=100, step=100, key="qtd_op")
        with col4:
            preco = st.number_input("Preço de Entrada (R$):", min_value=0.01, value=10.0, step=0.01, key="preco_op")
        ...
        if tipo in ["Call", "Put"]:
            ...
            with col_cod1:
                codigo = st.text_input("Digite o código EXATO da opção:", value=codigo_sugestao, key=f"codigo_{tipo}_{ticker_selecionado}").upper().strip()
            
            if not codigo: st.warning("⚠️ Código de opção é obrigatório para Call/Put")
        ...
        if st.button("💾 Gravar Operação", use_container_width=True, key="btn_gravar"):
            ...
            if tipo in ["Call", "Put"] and not codigo:
                st.error("❌ Código de opção é obrigatório")
        ...
        st.markdown("---")
        st.subheader("📋 Suas Operações (Tabela de Caixa)")
        
        if operacoes:
            df_ops = []
            for op in operacoes:
                # Busca direcional separada para Opção e Ação
                if op["tipo_ativo"] in ["Call", "Put"] and op["codigo_opcao"]:
                    ticker_busca = f"{op['codigo_opcao']}.SA"
                else:
                    ticker_busca = op["ticker"]
                
                # A nova função retorna o preço E o status da conexão
                preco_atual, status_api = calcular_preco_com_status(ticker_busca, op["preco_compra"])
                ...
                # Ícone visual para você saber de onde veio o preço
                icone = "🟢 " if status_api == "API" else "⚠️ "
                ...
                df_ops.append({
                    ...
                    "Preço Atual": f"{icone}R$ {preco_atual:.2f}",
                    "L/P (R$)": f"R$ {lucro_prejuizo:.2f}",
                    "Var (%)": f"{percentual:.2f}%"
                })
            
            st.dataframe(pd.DataFrame(df_ops), use_container_width=True, hide_index=True)
            st.caption("🟢 Preço da B3 em Tempo Real | ⚠️ Preço Congelado (Sem dados da API - Use Editar para atualizar manualmente)")
            
            st.markdown("---")
            st.subheader("✏️ Gerenciar e Atualizar Preços Manuais")
            
            col_edit, col_del = st.columns(2)
            with col_edit:
                st.write("**Atualizar Opções e Fechar Posições:**")
                op_formatada = [f"{op['codigo_opcao'] if op['codigo_opcao'] else op['ticker'].replace('.SA','')} - Qtd: {op['qtd']}" for op in operacoes]
                op_edit_idx = st.selectbox("Selecione a ordem:", range(len(operacoes)), format_func=lambda x: op_formatada[x], key="edit")
                
                op_selecionada = operacoes[op_edit_idx]
                
                status_edit = st.selectbox("Status:", ["Aberta", "Fechada"], key="status_edit")
                st.info("💡 Para opções (⚠️), digite o valor de mercado atual abaixo para calcular o lucro correto.")
                preco_edit = st.number_input("Preço Atualizado / Fechamento (R$):", value=op_selecionada["preco_compra"], key="preco_edit")
                qtd_edit = st.number_input("Nova Qtd:", value=op_selecionada["qtd"], key="qtd_edit")
                
                if st.button("✅ Atualizar Caixa", key="btn_edit"):
                    if atualizar_operacao(op_selecionada["id"], status_edit, preco_edit, int(qtd_edit)):
                        st.success("✅ Atualizado!")
                        st.rerun()
            
            with col_del:
                st.write("**Deletar Erros:**")
                ...
                if st.button("🗑️ EXCLUIR", key="btn_delete"):
    ...
    with tab2:
        st.subheader("🎯 Consenso de Analistas B3")
        df_recomendacoes = [{"Ação": t.replace(".SA", ""), "Empresa": d["Empresa"], "Setor": d["Setor"], "Consenso": d["Consenso"], "Alvo (R$)": f"R$ {d['Alvo']:.2f}", "Upside (%)": f"{d['Upside']:.1f}%"} for t, d in banco_b3.items()]
        df_rec = pd.DataFrame(df_recomendacoes)
        
        c_f1, c_f2 = st.columns(2)
        with c_f1: consenso_filter = st.multiselect("Consenso:", ["Alta", "Neutro", "Baixa"], default=["Alta", "Neutro", "Baixa"])
        with c_f2: setor_filter = st.multiselect("Setor:", df_rec["Setor"].unique(), default=df_rec["Setor"].unique())
        
        st.dataframe(df_rec[(df_rec["Consenso"].isin(consenso_filter)) & (df_rec["Setor"].isin(setor_filter))], use_container_width=True, hide_index=True)
    
    # ===== TAB 3: CARTEIRA =====
    with tab3:
        st.subheader("💼 Análise de Exposição da Carteira")
        
        if operacoes:
            df_carteira = []
            inv_total, val_atual_total = 0, 0
            ...
            for op in operacoes:
                ...
                inv_total += investimento
                val_atual_total += valor_atual
                ...
                df_carteira.append({
                    "Ativo": display_ticker,
                    "Qtd": op["qtd"],
                    "Valor Investido": f"R$ {investimento:.2f}",
                    "Valor Atual": f"{icone}R$ {valor_atual:.2f}",
                    "Lucro/Prejuízo (R$)": f"R$ {lucro:.2f}"
                })
            
            c_m1, c_m2, c_m3 = st.columns(3)
            with c_m1: st.metric("Investimento Total (Caixa)", f"R$ {inv_total:.2f}")
            with c_m2: st.metric("Valor Atual Mercado", f"R$ {val_atual_total:.2f}")
            with c_m3: st.metric("Lucro/Prejuízo Total", f"R$ {(val_atual_total - inv_total):.2f}")
    ...
    with tab4:
        ...
        if operacoes:
            ...
            with c_r2:
                ...
                fig2 = px.bar(x=status_counts.index, y=status_counts.values, title="Operações por Status", color=status_counts.index)
